...
```bash
    $ make update
```
:information_source: If you want to enable consul dns service, edit following file:

`    $ vi platform-ansible/defaults/env.yml`

>Change following lines, considering env_name accepting only [a-z] characters:
```YAML
env_name: envname
use_custom_dns: true
```

:information_source: If you want to decide on kerberos use, edit following file:

`    $ vi platform-ansible/defaults/cdh.yml`
